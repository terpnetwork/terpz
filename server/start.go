...
type StartCmdOptions struct {
	...
	PostSetup func(svrCtx *terpserver.Context, clientCtx client.Context, ctx context.Context, g *errgroup.Group) error
	// PostSetupStandalone can be used to setup extra services under the same cancellable context,
	PostSetupStandalone func(svrCtx *terpserver.Context, clientCtx client.Context, ctx context.Context, g *errgroup.Group) error
	// AddFlags add custom flags to start cmd
	AddFlags func(cmd *cobra.Command)
	// StartCommandHanlder can be used to customize the start command handler
	StartCommandHandler func(svrCtx *terpserver.Context, clientCtx client.Context, appCreator types.AppCreator, inProcessConsensus bool, opts StartCmdOptions) error
}
...
func start(svrCtx *terpserver.Context, clientCtx client.Context, appCreator types.AppCreator, withCmt bool, opts StartCmdOptions) error {
...
func startStandAlone(svrCtx *terpserver.Context, svrCfg serverconfig.Config, clientCtx client.Context, app types.Application, metrics *telemetry.Metrics, opts StartCmdOptions) error {
	addr := svrCtx.Viper.GetString(flagAddress)
	transport := svrCtx.Viper.GetString(flagTransport)

	cmtApp := terpserver.NewCometABCIWrapper(app)
...
func startInProcess(svrCtx *terpserver.Context, svrCfg serverconfig.Config, clientCtx client.Context, app types.Application,
...
func getAndValidateConfig(svrCtx *terpserver.Context) (serverconfig.Config, error) {
...
func setupTraceWriter(svrCtx *terpserver.Context) (traceWriter io.WriteCloser, cleanup func(), err error) {
...
func wrapCPUProfile(svrCtx *terpserver.Context, callbackFn func() error) error {
...
func getCtx(svrCtx *terpserver.Context, block bool) (*errgroup.Group, context.Context) {
	ctx, cancelFn := context.WithCancel(context.Background())
	g, ctx := errgroup.WithContext(ctx)
	// listen for quit signals so the calling parent process can gracefully exit
	terpserver.ListenForQuitSignals(g, block, cancelFn, svrCtx.Logger)
	return g, ctx
}

func startApp(svrCtx *terpserver.Context, appCreator types.AppCreator, opts StartCmdOptions) (app types.Application, cleanupFn func(), err error) {
	...
	db, err := opts.DBOpener(home, terpserver.GetAppDBBackend(svrCtx.Viper))
	if err != nil {
		return app, traceCleanupFn, err
	}
